...
export function formatChatFetchError(err: unknown): string {
  ...
  if (msg === "Failed to fetch" || msg.includes("NetworkError")) {
    return (
      "Could not reach the chat service. Sign in, deploy latest Supabase edge functions " +
      "(CORS for shadowtalk://), and rebuild the desktop app with .env set. See DESKTOP.md."
    );
  }
  return msg;
}
